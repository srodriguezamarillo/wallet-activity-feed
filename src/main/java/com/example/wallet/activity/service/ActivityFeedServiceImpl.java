...
public class ActivityFeedServiceImpl implements ActivityFeedService
{
	...
	{
		log.debug("Fetching activity feed for user: {}, filters: product={}, status={}, currency={}, page={}, size={}",
		...
		log.debug("Found {} activities for user: {}", result.getTotalElements(), userId);
	...
	{
		log.debug("Fetching activity detail for id: {}", id);
		
		ActivityEvent event = repository.findById(id).orElseThrow(() -> {
			log.warn("Activity not found: {}", id);
			return new ActivityNotFoundException("Activity not found: " + id);
		});
